...
def get_request_password_reset_use_case(
    user_repo: Annotated[IUserRepository, Depends(get_user_repository)],
    token_service: Annotated[ITokenService, Depends(get_jwt_service)],
    message_broker: Annotated[IMessageBroker, Depends(get_message_broker)],
) -> UseCase:
    return RequestPasswordResetUseCase(user_repo, token_service, message_broker)
...
async def get_reset_password_use_case(
    ...
    return ResetPasswordUseCase(user_repo, token_service, password_hasher, blacklist_service)
