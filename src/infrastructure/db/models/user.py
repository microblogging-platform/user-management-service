...
class UserModel(Base):
    ...
    id: Mapped[UUID] = mapped_column(
        primary_key=True, default=uuid4, server_default=None
    )
    ...
    role: Mapped[Role] = mapped_column(
        SQLEnum(
            Role, native_enum=False, values_callable=lambda x: [e.value for e in Role]
        ),
        nullable=False,
        default=Role.USER,
    )
    ...
    group: Mapped[Optional["GroupModel"]] = relationship(
        "GroupModel", back_populates="users", lazy="selectin"
    )
